"""
notebooks/04_lstm_model.py
───────────────────────────
Phase 3: LSTM forecasting on a single crop-state series.
...
def main(crop: str, state: str, steps: int, auto_select_best: bool = True) -> None:
    ...
    print("\n" + "=" * 60)
    print("PHASE 4 — LSTM SEQUENCE MODEL")
    print(f"Crop: {crop} | State: {state}")
    print("=" * 60)

    model = LSTMPriceForecaster(crop=crop, state=state, epochs=10, lookback=30, max_samples=5000)
    model.fit(series)
    forecast = model.forecast(steps=steps)

    model_path = model.save()
    forecast_path = OUTPUT_DIR / f"lstm_forecast_{crop.lower().replace(' ', '_')}_{state.lower().replace(' ', '_')}.csv"
    ...
    ax.set_title(f"LSTM forecast: {crop} @ {state}")
    ax.set_ylabel("₹/quintal")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"₹{x:,.0f}"))
    ax.legend()
    plt.tight_layout()
    fig_path = OUTPUT_DIR / f"lstm_forecast_{crop.lower().replace(' ', '_')}_{state.lower().replace(' ', '_')}.png"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Phase 3: LSTM model with auto state selection"
    )
    ...
    main(args.crop, args.state, args.steps, auto_select_best=not args.no_auto_select)
